...
#             poblaciones.append((nombre, codigo, int(año), int(censo)))
#     return poblaciones
            
############################################################################################
#VERSION POR COMPRESION#
########################
    with open(fichero, encoding = 'utf-8') as f:
        lector = csv.reader(f)
        poblaciones = [Registro(nombre, codigo, int(año), int(censo)) for nombre, codigo, año, censo in lector]
    return poblaciones
...
def calcula_paises(poblaciones):
    ...
    '''
    conj = set(p.nombre for p in poblaciones)
    return sorted(conj)
# return sorted({r.nombre for r in poblaciones})
...
def filtra_por_pais(poblaciones, pais):
    ...
    '''
    conj = set((p.año, p.censo) for p in poblaciones if p.nombre == pais or p.codigo == pais)
    return sorted(conj)

# Solucion alternativa
#     filtro = [(p.año, p.censo) for p in poblaciones if p.nombre == pais or p.codigo == pais]
#     return filtro
##############################################################################################
...
def filtra_por_paises_y_anyo(poblaciones, anyo, paises):
    ...
    '''
#     conj = set((p.nombre, p.censo) for p in poblaciones if p.año == anyo and (p.nombre in paises or p.codigo in paises))
#     return sorted(conj)
    return [(p.nombre, p.censo) for p in poblaciones if anyo == p.año and p.nombre in paises]
...
def muestra_evolucion_poblacion(poblaciones, pais):
    ...
    una lista de años y otra lista con los número de habitantes (con el mismo orden):
        ...
        plt.show()
    '''
    titulo = 'Ev poblacion del pais: ' + pais
    poblaciones_filtradas = filtra_por_pais(poblaciones, pais)
    l_anyos=[año for (año,_) in poblaciones_filtradas]
    l_habitantes=[censo for (_,censo) in poblaciones_filtradas]
...
def muestra_evolucion_poblaciones(poblaciones, paises):
    ''' Genera una curva con la evolución de la población de un país. El pais puede
    darse como su nombre completo o por su código.
    ...
    una lista de años y otra lista con los número de habitantes (con el mismo orden):
        ...
        plt.show()
    '''
    titulo = 'Comparacion de la evolucion entre los paises'
    for pais in paises:
        poblaciones_filtradas = filtra_por_pais(poblaciones, pais)
        l_anyos=[año for (año,_) in poblaciones_filtradas]
        l_habitantes=[censo for (_,censo) in poblaciones_filtradas]
        plt.plot(l_anyos,l_habitantes, label = pais)
...
def muestra_comparativa_paises_anyo(poblaciones, anyo, paises):
    ...
    filtro = filtra_por_paises_y_anyo(poblaciones, anyo, paises)
    l_paises = [pais for (pais,_) in filtro]
    l_habitantes = [censo for (_,censo) in filtro]
